...
def raw_to_dicts(monuments_raw):
    ...
    for raw in monuments_raw_copy:
        ...
        tier2 = tier(2, int(raw[9]), int(raw[10]), parse_modifs(raw[11]), parse_modifs(
            raw[12]), parse_modifs(raw[13]), parse_modifs(raw[14]))
        tier3 = tier(3, int(raw[15]), int(raw[16]), parse_modifs(
            raw[17]), parse_modifs(raw[18]), parse_modifs(raw[19]), parse_modifs(raw[20]))
